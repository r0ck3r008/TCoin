defmodule Honeycomb_rand.Worker do
  ...
  def start_link(num, disp_pid, agnt_pid, frbdn) do
    ...
    nbor_dir=mk_nbor_dir(agnt_pid, nbors, %{co_ords=>self()}, Enum.count(nbors))
  ...
  def fetch_co_ords(num, disp_pid, agnt_pid, frbdn, nil) do
    ...
    fetch_co_ords(
      ...
      agnt_pid,
      frbdn,
      Honeycomb_rand.Dispenser.chk_co_ord(disp_pid, co_ords, agnt_pid, self())
    )
  end
  def fetch_co_ords(_num, _disp_pid, _agnt_pid, _frbdn, co_ords), do: co_ords
  ...
  def find_nbors(co_ords, num, flag, frbdn, nil) do
    find_nbors(
      co_ords,
      num,
      flag,
  ...
  def find_nbors(_co_ords, _num, _flag, _frbdn, nbors), do: Enum.filter(nbors, fn(x)->!is_nil(x) end)
